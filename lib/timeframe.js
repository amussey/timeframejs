(function ($) {
	...
	Timeline.prototype.addSpan = function (spanData, category) {
		...
		if (spanData.text || this.underText) {
			span.text = parseTextString(spanData.text);
			this.underText = true;
			this.levelSize = 40;
		}
	...
	Timeline.prototype.calculateLevels = function () {
		for (var c = 0; c < this.categories.length; c++) {
			var category = this.categories[c];

			this.levels = 0;

			for (var s1 = 0; s1 < category.spans.length; s1++) {
				category.spans[s1].level = 0;
				var offlimitLevels = [];
				var levelSet = false;
				for (var s2 = 0; s2 < s1; s2++) {
					if (
						(category.spans[s1].start >= category.spans[s2].start && category.spans[s1].start < category.spans[s2].end) ||
						(category.spans[s1].end >= category.spans[s2].start && category.spans[s1].end < category.spans[s2].end) ||
						(category.spans[s1].start <= category.spans[s2].start && category.spans[s1].end > category.spans[s2].end)) {

						if (!(offlimitLevels.includes(category.spans[s2].level))) {
							offlimitLevels.push(category.spans[s2].level);
						}
					}
				}

				while (!levelSet) {
					if (offlimitLevels.includes(category.spans[s1].level)) {
						category.spans[s1].level += 1;
					} else {
						levelSet = true;
					}
				}
				if (this.levels < category.spans[s1].level) {
					this.levels = category.spans[s1].level;
				}
			}
		}
		this.container.outerHeight(this.container.outerHeight() + (this.levels * this.levelSize) + (this.underText ? this.underTextPadding : 0));
		this.svg.outerHeight(this.svg.outerHeight() + (this.levels * this.levelSize) + (this.underText ? this.underTextPadding : 0));
	...
	Timeline.prototype.drawBackground = function () {
		...
		var bottomLine = $svg.line(
			this.paddingX,
			this.height + this.paddingY - textHeight - tickHeight - 0.5 + (this.levels * this.levelSize) + (this.underText ? this.underTextPadding : 0),
			...
			for (var year = this.startDate.getFullYear(); year <= this.endDate.getFullYear(); year++) {
				...
				if (isMajorTick) {
					var yearLabel = $svg.text(
						x,
						this.height + this.paddingY + (this.levels * this.levelSize) + (this.underText ? this.underTextPadding : 0),
						year,
						{
							fill: "black",
							"font-size": "12",
				...
				var yearTick = $svg.line(
					x,
					this.height + this.paddingY - textHeight - tickHeight + (this.levels * this.levelSize) + (this.underText ? this.underTextPadding : 0),
					x,
					this.height + this.paddingY - textHeight + (this.levels * this.levelSize) + (this.underText ? this.underTextPadding : 0)
				)
					.attr("stroke", "#000000")
					.attr("stroke-width", 1)
					.appendTo(background);
	...
	Timeline.prototype.drawEvent = function (event) {
		...
		var underText = $svg.text(
			x,
			this.height - this.options.textHeight - this.options.tickHeight + this.paddingY - 8 +10,
			event.text,
			{
				fill: "black",
				"font-size": "12",
	...
	Timeline.prototype.drawSpan = function (span) {
		...
		var rect = $svg.rect(
			startX,
			this.height - this.options.textHeight - this.options.tickHeight + this.paddingY - 4 + ((this.levels - span.level) * this.levelSize),
			endX - startX,
			7,
			{
				rx: 5,
		...
		var label = $svg.text(
			(startX + endX) / 2,
			this.height - this.options.textHeight - this.options.tickHeight + this.paddingY - 8  + ((this.levels - span.level) * this.levelSize),
			span.name,
			{
				fill: "black",
				"font-size": "12",
		...
		var underText = $svg.text(
			(startX + endX) / 2,
			this.height - this.options.textHeight - this.options.tickHeight + this.paddingY - 8  + ((this.levels - span.level) * this.levelSize) + 22,
			span.text,
			{
				fill: "black",
				"font-size": "12",
	...
	var parseTextString = function (textString) {

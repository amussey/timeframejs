(function ($) {
	...
	Timeline.prototype.calculateLevels = function () {
		...
		this.container.outerHeight(this.container.outerHeight() + (this.levels * this.levelSize));
		this.svg.outerHeight(this.svg.outerHeight() + (this.levels * this.levelSize));
	...
	Timeline.prototype.drawBackground = function () {
		...
		var bottomLine = $svg.line(
			this.paddingX,
			this.height + this.paddingY - textHeight - tickHeight - 0.5 + (this.levels * this.levelSize),
			...
			for (var year = this.startDate.getFullYear(); year <= this.endDate.getFullYear(); year++) {
				...
				if (isMajorTick) {
					var yearLabel = $svg.text(
						x,
						this.height + this.paddingY + (this.levels * this.levelSize),
						year,
						{
							fill: "black",
							"font-size": "12",
				...
				var yearTick = $svg.line(
					x,
					this.height + this.paddingY - textHeight - tickHeight + (this.levels * this.levelSize),
					x,
					this.height + this.paddingY - textHeight + (this.levels * this.levelSize)
				)
					.attr("stroke", "#000000")
					.attr("stroke-width", 1)
					.appendTo(background);

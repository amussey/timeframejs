(function ($) {
			...
			for (var s1 = 0; s1 < category.spans.length; s1++) {
				...
				for (var s2 = 0; s2 < s1; s2++) {
					if (
						...
						if (this.maxLevels < category.spans[s1].level) {
							this.maxLevels = category.spans[s1].level;
						}
					}
				}
			}
	...
	Timeline.prototype.drawSpan = function (span) {
		...
		var rect = $svg.rect(
			startX,
			this.height - this.options.textHeight - this.options.tickHeight + this.paddingY - 4 - (span.level * 24),
			endX - startX,
			7,
			{
				rx: 5,
		...
		var label = $svg.text(
			(startX + endX) / 2,
			this.height - this.options.textHeight - this.options.tickHeight + this.paddingY - 8 - (span.level * 24),
			span.name,
			{
				fill: "black",
				"font-size": "12",
